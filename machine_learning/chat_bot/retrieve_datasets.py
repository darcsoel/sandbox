...
class DialogsRetriever:
    ...
    def _get_json_files_list(self):
        all_files = [f for f in os.listdir(self._root_dir) if os.path.isfile(os.path.join(self._root_dir, f))]
        return [f for f in all_files if re.search(self.dataset_file_extension_pattern, f)]
